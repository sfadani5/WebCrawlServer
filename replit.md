...
| DB | SQLite3 |
| 실시간 통신 | WebSocket (`ws`) |
| 워크플로우 | js-yaml, node-cron, scriptEngine |

## 실행 방법

```bash
npm install
npm start        # node server/start.mjs (포트 9600)
...
| 경로 | 역할 |
|------|------|
| `server/start.mjs` | 서버 진입점 |
| `modules/` | 확장 모듈 (각 모듈은 actions/, workflow.yaml 등 포함) |
| `plugin/` | 브라우저 플러그인 소스 |
| `server/admin-ui/dist` | 관리자 페이지 빌드 산출물 (React) |
...
- `AGENTS.md` — AI 운영 규칙
- `docs/rule/R-00000 instructions.md` — 세부 개발 규칙
- `docs/ask.md` — 작업 요청
